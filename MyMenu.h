...
namespace game {
	class MyMenu
	{
		...
		int rast_m;                                    // Расстояние между пунктами меню
	...
	public:
		...
		MyMenu(sf::RenderWindow& window, float menux, float menuy, int sizeFont, int step, std::vector<sf::String>& name);

		void draw();																				// Рисуем меню
		void MoveUp();																				// Перемещение выбора меню вверх
		void MoveDown();																			// Перемещение выбора меню вниз
		void setColorTextMenu(sf::Color menColor, sf::Color ChoColor, sf::Color BordColor);			// Цвет элементов игрового меню
